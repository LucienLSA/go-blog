...
func SendEmailCodeHandler(c *gin.Context) {
	...
	// 2. 发送邮箱验证码业务
	err := service.SendEmailCode(p)
	if err != nil {
		zap.L().Error("service SendEmailCode failed", zap.String("UserEmail", p.UserEmail), zap.Error(err))
		if errors.Is(err, mysql.ErrorEmailNotExit) {
...
// 	}
// 	// 2. 业务处理
// 	user, err := service.SendEmailCode(p)
// 	if err != nil {
// 		zap.L().Error("service login failed", zap.String("UserEmail", p.UserEmail), zap.Error(err))
// 		if errors.Is(err, mysql.ErrorUserNotExist) {
// 			response.ResponseError(c, response.CodeUserNotExist)
// 			return
// 		}
