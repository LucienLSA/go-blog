...
func SetupRouter(mode string) *gin.Engine {
	...
	// 使用增强的ginlogger中间件，支持TraceID和用户信息关联
	r.Use(logging.EnhancedGinLogger(), logging.GinRecovery(true))
	// // 存入cookie
	...
	userRoute := v2.Group("/user")
	{
		...
		// 需要认证的路由 - 创建新的认证路由组
		userAuthRoute := userRoute.Group("/auth")
		userAuthRoute.Use(middlewares.JWTAuthMiddleware())
		{
			// 用户头像上传
	...
	communityRoute := v2.Group("/community")
	{
		...
		// 需要认证的路由 - 创建新的认证路由组
		communityAuthRoute := communityRoute.Group("/auth")
		communityAuthRoute.Use(middlewares.JWTAuthMiddleware())
		{
			// 创建社区
			communityAuthRoute.POST("/create", controller.CreateCommunityHandler())
		}
	}
	postsRoute := v2.Group("/posts")
	{
		// 不需要认证的公共路由
		postsRoute.GET("/test", func(c *gin.Context) {
			c.JSON(http.StatusOK, gin.H{
				"message": "postsRoute test route - no auth required",
		...
		// 需要认证的路由 - 创建新的认证路由组
		postsAuthRoute := postsRoute.Group("/auth")
		postsAuthRoute.Use(middlewares.JWTAuthMiddleware())
		{
			// 帖子查询新版
			...
			// 帖子审核
			// 提交帖子审核（需要登录）
			postsAuthRoute.POST("/review", controller.SubmitPostForReviewHandler())
		}
	}
	...
	// 示例日志路由（用于测试增强日志功能）
	v2.GET("/example/logging", middlewares.JWTAuthMiddleware(), controller.ExampleLoggingHandler())
